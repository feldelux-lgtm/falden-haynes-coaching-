...
export default config({
  ...
  collections: {
    ...
    blog: collection({
      ...
      slugField: 'title',
      schema: {
        title: fields.slug({ name: { label: 'Title', description: 'The post headline. Keep under 70 characters for Google.' } }),
        excerpt: fields.text({
          label: 'Excerpt (card summary)',
          multiline: true,
          description: 'Shown on the blog listing card. 1–2 sentences, under 160 characters. Example: "Most founders skip this step and pay for it later. Here\'s why a business plan is worth doing before you spend a penny."',
        }),
        date: fields.date({ label: 'Published date', description: 'Pick the date this post goes live.' }),
        tag: fields.select({
          label: 'Category tag',
          description: 'Shown as a colour label on the card and at the top of the article.',
          options: [
            { label: 'Business Planning', value: 'Business Planning' },
            { label: 'Entrepreneurship', value: 'Entrepreneurship' },
            { label: 'Growth & Strategy', value: 'Growth & Strategy' },
            { label: 'Mindset & Leadership', value: 'Mindset & Leadership' },
        ...
        featured_image_url: fields.text({
          label: 'Cover image URL',
          description: 'Go to pexels.com → find a photo → right-click the image → Copy image address. Paste it here, then add this to the end: ?auto=compress&cs=tinysrgb&w=1200&h=500&fit=crop — Recommended size: 1200 × 500 px. Example: https://images.pexels.com/photos/3184292/pexels-photo-3184292.jpeg?auto=compress&cs=tinysrgb&w=1200&h=500&fit=crop',
        }),
        read_time: fields.text({
          label: 'Read time',
          description: 'Example: 5 min read or 8 min read. Rough guide: 200 words per minute.',
          defaultValue: '5 min read',
        }),
        author: fields.text({ label: 'Author', defaultValue: 'Feldon Haynes' }),
        seo_title: fields.text({ label: 'SEO title (leave blank to use post title)', description: 'Under 60 characters. Example: "Benefits of a Business Plan | Feldon Haynes Coaching"' }),
        seo_description: fields.text({
          label: 'Meta description',
          multiline: true,
          description: 'Shown in Google search results. Under 160 characters. Example: "The real benefits of a business plan go beyond investor decks. Learn what a proper plan does for your business."',
        }),
        content: fields.document({
          label: 'Article content',
          description: 'HEADINGS: Heading 2 = main section title. Heading 3 = sub-point under a section. PARAGRAPHS: Leave a blank line between each paragraph. LISTS: Use bullet list for key points, numbered list for step-by-step. BOLD: Ctrl+B for key terms. QUOTE/CALLOUT: Use the blockquote button (") for a highlighted insight. LINKS: Highlight text → click the link button → paste URL.',
          formatting: {
            headingLevels: [2, 3],
